...
/**
 * Mika — an illustrated, gradient-shaded chibi character built entirely out of SVG
...
export const MikaFigure: React.FC<MikaFigureProps> = ({ size = 150, bounce = false, wave = false, className = "" }) => {
  ...
  return (
    ...
    >
      ...
      <ellipse cx="100" cy="322" rx="46" ry="9" fill="url(#mikaShadow)" />

      {/* Back twin-tails (behind body) */}
      <path d="M62 96 C40 120 34 170 46 222 C52 210 60 200 64 190 C56 160 58 122 62 96 Z" fill="url(#mikaHair)" />
      <path d="M138 96 C160 120 166 170 154 222 C148 210 140 200 136 190 C144 160 142 122 138 96 Z" fill="url(#mikaHair)" />
      ...
      <motion.g style={{ transformOrigin: "72px 148px" }}>
        <rect x="63" y="145" width="16" height="62" rx="8" fill="url(#mikaSkin)" />
      </motion.g>
      ...
      >
        <rect x="121" y="145" width="16" height="62" rx="8" fill="url(#mikaSkin)" />
      </motion.g>

      {/* Neck */}
      <rect x="92" y="96" width="16" height="18" fill="url(#mikaSkin)" />

      {/* Head */}
      <circle cx="100" cy="76" r="40" fill="url(#mikaSkin)" />
      ...
      <path d="M58 66 C52 84 52 100 58 112 C64 100 64 84 66 68 Z" fill="url(#mikaHair)" />
      <path d="M142 66 C148 84 148 100 142 112 C136 100 136 84 134 68 Z" fill="url(#mikaHair)" />
      {/* Front bangs */}
      <path
...
/** Small round headshot crop of Mika, used in the chat panel header and message bubbles. */
export const MikaAvatar: React.FC<{ size?: number; className?: string }> = ({ size = 32, className = "" }) => {
  const [blink, setBlink] = useState(false);
  ...
  return (
    ...
    >
      <defs>
        <linearGradient id="mikaHairSmall" x1="0%" y1="0%" x2="100%" y2="100%">
          <stop offset="0%" stopColor={HAIR_LIGHT} />
          <stop offset="100%" stopColor={HAIR_DARK} />
        </linearGradient>
        <linearGradient id="mikaSkinSmall" x1="0%" y1="0%" x2="0%" y2="100%">
          <stop offset="0%" stopColor={SKIN} />
          <stop offset="100%" stopColor={SKIN_SHADE} />
        </linearGradient>
      </defs>
      <circle cx="100" cy="76" r="40" fill="url(#mikaSkinSmall)" />
      <path
        d="M58 70 C54 34 76 8 100 8 C124 8 146 34 142 70 C140 54 128 44 100 44 C72 44 60 54 58 70 Z"
        fill="url(#mikaHairSmall)"
      />
      <path d="M58 66 C52 84 52 100 58 112 C64 100 64 84 66 68 Z" fill="url(#mikaHairSmall)" />
      <path d="M142 66 C148 84 148 100 142 112 C136 100 136 84 134 68 Z" fill="url(#mikaHairSmall)" />
      <path
        d="M62 58 C70 40 86 30 100 30 C114 30 130 40 138 58 C126 48 112 44 100 44 C88 44 74 48 62 58 Z"
        fill="url(#mikaHairSmall)"
      />
